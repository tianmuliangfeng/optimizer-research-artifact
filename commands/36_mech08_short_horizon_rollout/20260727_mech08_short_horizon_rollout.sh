...
SCRIPT_DIR="${REPO}/scripts/36_mech08_short_horizon_rollout"
OUTPUT_ROOT="${RESULTS}/36_mech08_short_horizon_rollout"
...
# Space-separated physical GPU ids, for example:
#   MECH08_GPUS="0 1" bash commands/36_mech08_short_horizon_rollout/20260727_mech08_short_horizon_rollout.sh
read -r -a GPUS <<< "${MECH08_GPUS:-0}"
MAX_PARALLEL="${MECH08_MAX_PARALLEL:-${#GPUS[@]}}"

"${CONTROLLER_PY}" "${SCRIPT_DIR}/test_mech08_contract.py"
"${CHILD_PY}" "${SCRIPT_DIR}/test_mech08_worker.py"

COMMAND=(
  "${CONTROLLER_PY}"
  "${SCRIPT_DIR}/run_mech08.py"
  --output-root "${OUTPUT_ROOT}"
  --contract "${SCRIPT_DIR}/rollout_contract.json"
  --prediction-reference "${SCRIPT_DIR}/mech07_prediction_reference.csv"
  ...
  --execution-domain "llama-host-llama1b-mech08"
)

if [[ -n "${MECH08_RESUME_STAMP:-}" ]]; then
  COMMAND+=(--resume-stamp "${MECH08_RESUME_STAMP}")
fi

printf 'MECH-08 controller command:'
printf ' %q' "${COMMAND[@]}"
printf '\n'
"${COMMAND[@]}"
