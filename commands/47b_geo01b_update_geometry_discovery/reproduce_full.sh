...
LAUNCHER="${ROOT}/commands/47b_geo01b_update_geometry_discovery/20260804_ex47b_geo01b_discovery.sh"
RESULTS="${SNM_RESULTS_ROOT:-${ROOT}/runs}"
STAMP="$(date -u +%Y%m%dT%H%M%S+0000)"

export SNM_ARTIFACT_ROOT="${SNM_ARTIFACT_ROOT:-${ROOT}}"
export RUN_DIR="${RUN_DIR:-${RESULTS}/_shared/analysis/method_deepening_geo01b_update_geometry_discovery/${STAMP}}"
bash "${LAUNCHER}" check
bash "${LAUNCHER}" discovery
