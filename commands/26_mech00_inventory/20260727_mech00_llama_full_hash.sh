...
OUT_DIR="$EXPERIMENT_ROOT/26_mech00_inventory/llama_host_${STAMP}_full"
...
require_file "$RUNNER"
require_dir "$EXPERIMENT_ROOT/17_llama_swiglu_validation/results"
require_dir "$EXPERIMENT_ROOT/20_llama_swiglu_1b/medium"
require_dir "$EXPERIMENT_ROOT/20_llama_swiglu_1b/formal"
require_dir "$EXPERIMENT_ROOT/20_llama_swiglu_1b/multiseed_followup"
require_dir "$EXPERIMENT_ROOT/21_gpt_r1_host_bridge/results"
...
echo "Starting read-only MECH-00 full hash on the LLaMA host."
echo "Output: $OUT_DIR"
...
"$CTRL_PY" "$RUNNER" \
  ...
  --input "llama124=$EXPERIMENT_ROOT/17_llama_swiglu_validation/results" \
  --input "llama1b_medium=$EXPERIMENT_ROOT/20_llama_swiglu_1b/medium" \
  --input "llama1b_formal=$EXPERIMENT_ROOT/20_llama_swiglu_1b/formal" \
  --input "llama1b_followup=$EXPERIMENT_ROOT/20_llama_swiglu_1b/multiseed_followup" \
  --input "bridge=$EXPERIMENT_ROOT/21_gpt_r1_host_bridge/results" \
  --family-hint llama124=llama_124m \
  --family-hint llama1b_medium=llama_1b \
  --family-hint llama1b_formal=llama_1b \
  --family-hint llama1b_followup=llama_1b \
  --family-hint bridge=gpt_bridge \
  --repo "llama_source=${SNM_OFFICIAL_REPO}" \
  --repo "bridge_source=${SNM_OFFICIAL_REPO}" \
  --methods down_none none muon \
