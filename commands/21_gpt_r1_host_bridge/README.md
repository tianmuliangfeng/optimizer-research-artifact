# Commands for 21 Gpt R1 Host Bridge

- Status: `implemented`
- Code: `../../scripts/21_gpt_r1_host_bridge`
...
```bash
python reproducibility/reproduce.py inspect 21_gpt_r1_host_bridge
```
...
```bash
python reproducibility/reproduce.py reproduce 21_gpt_r1_host_bridge
```
...
python reproducibility/reproduce.py verify \
  21_gpt_r1_host_bridge --results-root /path/to/results \
  --run-dir /path/to/results/21_gpt_r1_host_bridge/RUN_ID
```

## Entrypoints

- `script:run_gpt_r1_host_bridge` -> `scripts/21_gpt_r1_host_bridge/run_gpt_r1_host_bridge.py` (reproduce)
