# Commands for 28 Wikitext Depth Kmode

- Status: `implemented`
- Code: `../../scripts/28_wikitext_depth_kmode`
...
```bash
python reproducibility/reproduce.py inspect 28_wikitext_depth_kmode
```
...
```bash
python reproducibility/reproduce.py reproduce 28_wikitext_depth_kmode --arg=--formal
```
...
python reproducibility/reproduce.py verify \
  28_wikitext_depth_kmode --results-root /path/to/results \
  --run-dir /path/to/results/28_wikitext_depth_kmode/RUN_ID
```

## Entrypoints

- `script:run_wikitext_depth_kmode` -> `scripts/28_wikitext_depth_kmode/run_wikitext_depth_kmode.py` (reproduce; requires receipt-bound --arg values)
