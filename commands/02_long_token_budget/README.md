# Commands for 02 Long Token Budget

- Status: `implemented`
- Code: `../../scripts/02_long_token_budget`
...
```bash
python reproducibility/reproduce.py inspect 02_long_token_budget
```
...
```bash
python reproducibility/reproduce.py reproduce 02_long_token_budget
```
...
python reproducibility/reproduce.py verify \
  02_long_token_budget --results-root /path/to/results \
  --run-dir /path/to/results/02_long_token_budget/RUN_ID
```

## Entrypoints

- `script:run_long_budget_100m` -> `scripts/02_long_token_budget/run_long_budget_100m.py` (reproduce)
