...
# This is a paper-matched single-GPU adaptation of the frozen upstream recipe.
# Default to physical GPU1 so experiment 43 may independently use GPU0.

REPO="${REPO:-${SNM_REPO}}"
INPUT_ROOT="${INPUT_ROOT:-${SNM_RESULTS_ROOT}}"
EXPERIMENT_ROOT="${INPUT_ROOT}/44_newton_muon_record17_455m"
...
LIVE_SUITE_SCRIPT="${REPO}/scripts/44_newton_muon_record17_455m/run_record17_suite.py"
SNAPSHOT_SUITE_SCRIPT="${RUN_DIR}/source_snapshot/controller/run_record17_suite.py"
...
if [[ ! -f "${SUITE_SCRIPT}" ]]; then
  echo "Experiment-44 suite controller is missing: ${SUITE_SCRIPT}" >&2
  exit 2
fi
...
if [[ "${#GPU_ARGS[@]}" -lt 1 || "${#GPU_ARGS[@]}" -gt 2 ]]; then
  echo "EXP44_GPUS must specify one or two unique physical GPU indices: ${EXP44_GPUS}" >&2
  exit 2
fi
if [[ "${#GPU_ARGS[@]}" -eq 2 && "${GPU_ARGS[0]}" == "${GPU_ARGS[1]}" ]]; then
  echo "EXP44_GPUS contains a duplicate lane: ${EXP44_GPUS}" >&2
  exit 2
fi
for gpu in "${GPU_ARGS[@]}"; do
  if [[ ! "${gpu}" =~ ^(0|[1-9][0-9]*)$ ]]; then
    echo "EXP44_GPUS must use canonical physical indices, not UUID aliases: ${EXP44_GPUS}" >&2
    exit 2
  fi
done
...
recovery_command() {
  ...
  if [[ -f "${SNAPSHOT_SUITE_SCRIPT}" && -f "${SNAPSHOT_MANIFEST}" ]]; then
    ...
    recovery=(
      ...
      "TRAIN_PY=${TRAIN_PY}"
      bash
      "${REPO}/commands/44_newton_muon_record17_455m/20260730_newton_muon_record17_455m.sh"
    )
  fi
  printf '%q ' "${recovery[@]}"
}
...
on_exit() {
  code=$?
  if [[ "${code}" -ne 0 ]]; then
    echo "Experiment 44 stopped with exit code ${code}." >&2
...
echo "Experiment 44 controller started"
...
echo "Experiment 44 completed."
echo "Artifacts: ${RUN_DIR}"
echo "Manifest: ${RUN_DIR}/record17_suite_manifest.json"
