...
LAUNCHER="${ROOT}/commands/47_update_geometry_curvature/20260804_ex47_update_geometry_curvature.sh"
RESULTS="${SNM_RESULTS_ROOT:-${ROOT}/runs}"
STAMP="$(date -u +%Y%m%dT%H%M%S+0000)"

export SNM_ARTIFACT_ROOT="${SNM_ARTIFACT_ROOT:-${ROOT}}"
export RUN_DIR="${RUN_DIR:-${RESULTS}/_shared/analysis/method_deepening_geo01_update_curvature/${STAMP}}"
bash "${LAUNCHER}" check
bash "${LAUNCHER}" pilot
