# Commands for 03 Fixed Memory

- Status: `implemented`
- Code: `../../scripts/03_fixed_memory`
...
```bash
python reproducibility/reproduce.py inspect 03_fixed_memory
```
...
```bash
python reproducibility/reproduce.py reproduce 03_fixed_memory
```
...
python reproducibility/reproduce.py verify \
  03_fixed_memory --results-root /path/to/results \
  --run-dir /path/to/results/03_fixed_memory/RUN_ID
```

## Entrypoints

- `script:run_fixed_memory_batch_sweep` -> `scripts/03_fixed_memory/run_fixed_memory_batch_sweep.py` (reproduce)
