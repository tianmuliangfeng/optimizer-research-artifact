...
ROOT="${SNM_RESULTS_ROOT}/30_mech02_k_geometry/r1_native_endpoint/$STAMP"
...
PACKAGE="$PACKAGE_DIR/mech02_r1_endpoint_${STAMP}.tgz"
mkdir -p "$PACKAGE_DIR"
tar -C "$(dirname "$ROOT")" -czf "$PACKAGE" "$(basename "$ROOT")"
echo "MECH-02 R1 endpoint PASS: $FORMAL_OUT/mech02_manifest.json"
echo "MECH-02 R1 handoff: $PACKAGE"
sha256sum "$PACKAGE"
ls -lh "$PACKAGE"
