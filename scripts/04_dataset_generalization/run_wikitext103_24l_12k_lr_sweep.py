...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the WikiText-103 24L/12k dataset-by-scale learning-rate comparison."
    )
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=[0.01, 0.02],
        help="Matrix learning rates to compare; 0.01 is the preselected primary setting.",
    )
    ...
    parser.add_argument("--run-prefix", default="mainconf_wikitext103_24L_12k_lr_sweep")
...
def build_commands(
    ...
    commands = []
    consumed_tokens = args.max_iters * args.batch_size * args.block_size * args.gradient_accumulation_steps
    for muon_lr in args.muon_learning_rates:
        args.muon_learning_rate = muon_lr
        lr_label = f"mulr{str(muon_lr).replace('.', 'p')}"
        tags = (
            f"publication,dataset_generalization,scale_interaction,lr_sensitivity,"
            f"{args.dataset},steps_{args.max_iters},tokens_{consumed_tokens},"
            f"L{args.n_layer},D{args.n_embd},{lr_label}"
        )
        for seed in args.seeds:
            args.seed = seed
            group = args.wandb_group or f"{args.run_prefix}_{lr_label}_seed{seed}"
            for method in args.methods:
                if method in ("muon", "newton"):
                    method_label = "00_muon" if method == "muon" else "13_newton_muon_fast"
                    run_name = f"{args.run_prefix}_{lr_label}_{method_label}_seed{seed}"
                    ...
                    release_frac = (
                        args.middle_release_frac if args.dry_run else read_mask_target(middle_mask)
                    )
                    label = f"middle_cproj_{release_label(release_frac)}"
                    run_name = f"{args.run_prefix}_{lr_label}_{label}_seed{seed}"
                    ...
                    release_frac = (
                        args.release_all_cproj_frac
                        if args.dry_run
                        else read_mask_target(release_all_mask)
                    )
