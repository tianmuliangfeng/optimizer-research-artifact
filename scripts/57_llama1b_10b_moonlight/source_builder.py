#!/usr/bin/env python3
"""Derive EX57 Moonlight trainers from the accepted LLaMA/EX48 sources.
...
def audit_moonlight_transfer(root: Path, local_source: str) -> dict[str, object]:
    ...
    if not all(checks.values()):
        raise RuntimeError(f"EX57 Moonlight algorithm drifted from EX19: {checks}")
    return {
        "passed": True,
...
RNG_CPU_COMPAT_MARKER = "EX57_MOONLIGHT_RNG_CPU_BYTE_COMPAT_V1"
...
def derive_trainer(source: str) -> str:
    ...
    source = replace_once(
        ...
        "def ex57_moonlight_kwargs() -> dict[str, Any]:\n"
        "    required = ('MOONLIGHT_WEIGHT_DECAY', 'MOONLIGHT_CONTRACT_SHA256', 'MOONLIGHT_SELECTION_SHA256')\n"
        "    missing = [name for name in required if not os.environ.get(name)]\n"
        "    if missing:\n"
        "        raise RuntimeError(f'EX57 Moonlight environment is incomplete: {missing}')\n"
        "    return {\n"
        "        'momentum': 0.95,\n"
        "        'nesterov': True,\n"
        "        'ns_steps': 5,\n"
    ...
    source = replace_once(
        ...
        "        matrix_optimizer = R1MoonlightMuon(matrix, lr=args.matrix_lr, **ex57_moonlight_kwargs())\n"
        "    else:\n"
        "        matrix_optimizer = SharedInputNewtonMuon(",
    ...
    source = replace_once(
        ...
        '        "moonlight_selection_sha256": os.environ.get("MOONLIGHT_SELECTION_SHA256", ""),\n'
        '        "moonlight_data_inventory_sha256": os.environ.get("MOONLIGHT_DATA_INVENTORY_SHA256", ""),\n'
        '        "moonlight_hyperparameters": ex57_moonlight_kwargs() if args.method == "moonlight" else {},\n'
        '        "checkpoint_path": str(checkpoint_path) if checkpoint_path.is_file() else "",',
        "summary lineage and timing",
    )
    compile(source, "<ex57_llama_moonlight_trainer>", "exec")
    return source
...
def derive_long_worker(source: str) -> str:
    ...
    source = replace_once(
        ...
        '        "timing_ineligible_reason": "quality-run concurrency and transferred Moonlight tuning",\n'
        '        "moonlight_selection_sha256": os.environ.get("MOONLIGHT_SELECTION_SHA256", ""),\n'
        '        "moonlight_hyperparameters": base.ex57_moonlight_kwargs(),\n'
        '        "train_s": train_s,',
        "long-worker timing and selection lineage",
    )
    compile(source, "<ex57_moonlight_long_worker_parent>", "exec")
    return source
