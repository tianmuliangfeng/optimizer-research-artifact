#!/usr/bin/env python3
"""Pure-stdlib EX57 Moonlight contract, data, cursor, and metric helpers."""
...
SCHEMA = "ex57_llama1b_10b_moonlight_v2"
CHECKPOINT_SCHEMA = "ex48_llama1b_segment_checkpoint_v1"
PHASE_MANIFEST_SCHEMA = "ex48_llama1b_phase_manifest_v1"
UNIT_MANIFEST_SCHEMA = "ex57_llama1b_moonlight_unit_v1"
DATA_AUDIT_SCHEMA = "ex57_fineweb_full_hash_audit_v1"
ACCEPTED_DATA_PROJECTION_SCHEMA = "ex57_accepted_ex48_data_projection_v1"
...
def validate_contract(contract: dict[str, Any]) -> dict[str, bool]:
    ...
    tuning = contract.get("tuning", {}).get("1b", {})
    formal = contract.get("formal", {})
    execution = contract.get("execution", {})
    moonlight = contract.get("moonlight", {})
    cells = tuning.get("cells", [])
    expected_cells = [("lr0010", 0.001), ("lr0018", 0.0018), ("lr0030", 0.003)]
    checks = {
        "schema": contract.get("schema_version") == SCHEMA,
        "experiment": int(contract.get("experiment_number", -1)) == 57
        and contract.get("experiment_id") == "57_llama1b_10b_moonlight",
        ...
        "tuning": tuning.get("seed") == 5701
        and tuning.get("updates") == 1000
        and tuning.get("center_cell") == "lr0018"
        and [(row.get("id"), float(row.get("matrix_lr", -1))) for row in cells] == expected_cells
        ...
        and int(tuning.get("seed", -1)) not in {2024, 2025, 2026},
        "formal": formal.get("seeds") == [2024, 2025, 2026]
        and int(formal.get("units", -1)) == 3
        and formal.get("tracks") == ["1b_multibudget_10b"]
        and formal.get("accepted_1b_budget_ids") == [
            "tokens_3p2506b", "tokens_6p9694b", "tokens_approximately_10b"
        ]
        and formal.get("independent_of_ex54") is True,
        "execution": execution.get("physical_gpus") == [0, 1, 2]
        and int(execution.get("maximum_concurrent_training_processes", -1)) == 3
        and int(execution.get("tuning_parallel_workers", -1)) == 3
        and int(execution.get("formal_parallel_workers", -1)) == 3
        and execution.get("tuning_assignment") == "one_lr_cell_per_gpu"
        and execution.get("formal_assignment") == "one_seed_per_gpu"
        ...
        * int(training.get("gradient_accumulation_steps_1b", -1))
        ...
        "fairness": contract.get("fairness", {}) == {
            ...
            "same_formal_seeds_as_ex48": True,
            "same_three_budget_endpoints_as_ex48": True,
        ...
        "equal_cooldowns": len(endpoints) == 3 and all(
            int(row["target_step"]) - int(row["start_step"]) == 1800 for row in endpoints
        ),
        "endpoint_tokens": endpoint_tokens == [3250585600, 6969360384, 9999745024],
        ...
        "independence": contract.get("source_derivation", {}).get("independent_package") is True
        and contract.get("source_derivation", {}).get("forbidden_runtime_dependency") == "scripts/54_*"
        and contract.get("formal", {}).get("independent_of_ex54") is True,
    }
    return checks
...
def assert_contract(contract: dict[str, Any]) -> None:
    ...
    if not all(checks.values()):
        raise RuntimeError(f"experiment 57 contract failed: {checks}")
