...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run the M1-P0 local quadratic-score probe on one none/release84 "
            "training trajectory. Candidate directions are shadow diagnostics "
            "and never modify the training update."
        )
    )
    ...
    parser.add_argument("--exact-svd", action="store_true")
    parser.add_argument("--no-line-search", action="store_false", dest="line_search")
    parser.add_argument(
        "--no-heldout-line-search",
        action="store_false",
        dest="heldout_line_search",
    )
    parser.set_defaults(exact_hvp=True, line_search=True, heldout_line_search=True)
    ...
    parser.add_argument("--run-prefix", default="mainconf_quadprobe_p0")
...
def string_override(value: str) -> str:
    """Quote string-valued overrides for nanoGPT's literal_eval configurator."""
...
def validate_args(args: argparse.Namespace) -> None:
    ...
    invalid_layers = [layer for layer in args.probe_layers if not 0 <= layer < args.n_layer]
    if invalid_layers:
...
def build_command(args: argparse.Namespace) -> list[str]:
    ...
    tags = (
        f"publication,{args.dataset},mechanism,quadratic_score,exact_hvp,"
    ...
    cmd = base_train_cmd(
        ...
        run_name=run_name,
        group=group,
        method="cproj_quadratic_probe_p0",
        tags=tags,
    )
    ...
    cmd.extend(
        [
            ...
            (
                ...
                f"{bool_literal(args.heldout_line_search)}"
            ),
            (
...
def validate_command(args: argparse.Namespace, cmd: list[str]) -> None:
    required = {
        "cproj_k_mode": "none",
        "cproj_quadratic_probe_enabled": "True",
        "cproj_quadratic_probe_steps": string_override(csv_literal(args.probe_steps)),
        "cproj_quadratic_probe_layers": string_override(csv_literal(args.probe_layers)),
        "cproj_quadratic_probe_modes": string_override(csv_literal(args.probe_modes)),
        "cproj_quadratic_probe_batch_size": str(args.probe_batch_size),
        "cproj_quadratic_probe_block_size": str(args.probe_block_size),
        "cproj_quadratic_probe_line_search_multipliers": string_override(
            csv_literal(args.line_search_multipliers)
        ),
        ...
        "cproj_quadratic_probe_heldout_line_search": bool_literal(
            args.heldout_line_search
        ),
        "update_similarity_probe_enabled": "False",
    ...
    if args.mechanism_config not in cmd:
        raise RuntimeError("quadratic probe command is missing mechanism config")


def print_expected_outputs(args: argparse.Namespace) -> None:
    directions = len(args.probe_steps) * len(args.probe_layers) * len(args.probe_modes)
    splits = 2 if args.heldout_line_search else 1
    line_rows = (
    ...
    print(
        "Expected quadratic-probe output: "
        f"{directions} direction rows and {line_rows} line-search rows."
    )
    print(
        "The probe uses a none/release84 training trajectory; all five candidate "
        "directions are shadow-only and do not change optimizer state."
    )
