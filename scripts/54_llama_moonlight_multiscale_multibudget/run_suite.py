#!/usr/bin/env python3
"""Seal, tune, run, resume, analyze, and verify Experiment 54."""
...
CONTRACT_NAME = "ex54_contract.json"
...
# EX54 ends at 6.97B; 10B is intentionally outside this experiment scope.
TEN_B_PHASE_IDS: tuple[str, ...] = ()
DEVICE_BATCH_SIZE_1B = 8
GRADIENT_ACCUMULATION_STEPS_1B = 64
MEMORY_SAFE_PROTOCOL = "ex54_moonlight_ex48_geometry_v1"
MEMORY_CANARY_SEED = 5403
MEMORY_CANARY_STEPS = 0
PREFLIGHT_SCHEMA = "ex54_moonlight_preflight_v1"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--wandb-project", default="Selective-Newton-Muon-MainConf-EX54-LLaMA-Moonlight-20260819")
    ...
    if args.gpus != [0, 1]:
        parser.error("fresh experiment 54 requires physical GPUs 0 and 1")
...
def package_files(root: Path) -> list[Path]:
    ...
    command = root / "commands/54_llama_moonlight_multiscale_multibudget/20260819_ex54_llama_moonlight_multiscale_multibudget.sh"
    if command.is_file():
...
def snapshot_sources(run_dir: Path, live_root: Path) -> Path:
    ...
    if snapshot.exists():
        ...
        required = {
            ...
            for name in (
                "ex54_contract.json", "accepted_ex48_data_projection.json",
                "protocol.py", "run_suite.py", "analyze.py", "long_worker.py",
                "source_builder.py", "moonlight_optimizer.py", "frozen_controls.csv",
            )
        } | {
        ...
        if (
            ...
            raise RuntimeError("EX54 source snapshot is incomplete or changed")
        ...
        if (
            ...
            raise RuntimeError("EX54 snapshot has no accepted EX48 data projection")
        return snapshot
    live = check_sources(live_root)
    if not live["passed"]:
        raise RuntimeError(f"EX54 live source check failed: {live['checks']}")
    ...
    payload = {
        "schema_version": "ex54_moonlight_source_snapshot_v1",
        "created_at": now_iso(),
...
def bind_gpu_compile_cache(
    ...
    """Use one TorchInductor cache root per physical GPU.

    EX54 executes independent single-GPU tuning/formal jobs concurrently.
    Separating their torch.compile disk caches prevents cold-start workers from
    contending on one shared cache path. This changes scheduling/compile I/O
    only; model math, seeds, data, batch geometry, and optimizer state are
    unchanged. Quality timing remains ineligible.
    """
    cache_dir = (run_dir / "_compile_cache" / f"gpu{int(gpu)}").absolute()
    cache_dir.mkdir(parents=True, exist_ok=True)
    env["TORCHINDUCTOR_CACHE_DIR"] = str(cache_dir)
    env["EX54_EXPECTED_PHYSICAL_GPU"] = str(int(gpu))
    return env
...
def run_logged(command: list[str], env: dict[str, str], log: Path) -> int:
    ...
    with log.open("a", encoding="utf-8") as handle:
        ...
        binding = {
            ...
            "expected_physical_gpu": env.get("EX54_EXPECTED_PHYSICAL_GPU"),
            "torchinductor_cache_dir": env.get("TORCHINDUCTOR_CACHE_DIR"),
        }
        handle.write("EX54_GPU_BINDING " + json.dumps(binding, sort_keys=True) + "\n")
        handle.flush()
        ...
        handle.write(f"EX54_WORKER_PID {process.pid}\n")
        handle.flush()
...
'''
    ...
    # Preserve the frozen virtualenv/module search path. The runtime probe does
    # not need to replace PYTHONPATH merely to hash triton_kernels.py.
    ...
    for scale in ("124m", "1b"):
        ...
        for seed in contract["formal"]["seeds"]:
            ...
            if completed.returncode or len(lines) != 1:
                raise RuntimeError(f"EX54 init audit failed {scale}/{seed}: {completed.stdout}\n{completed.stderr}")
            ...
            if not all(checks.values()):
                raise RuntimeError(f"EX54 init mismatch {scale}/{seed}: {checks}")
...
def require_preflight_context(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if not all(checks.values()):
        raise RuntimeError(f"EX54 invocation no longer matches passed preflight: {checks}")
    return manifest
...
def preflight(args: argparse.Namespace) -> None:
    ...
    if existing.is_file() and P.read_json(existing).get("passed") is True:
        ...
        print(f"skip passed EX54 preflight: {run_dir}")
        return
    ...
    if not runtime.get("passed"):
        raise RuntimeError(f"EX54 runtime preflight failed: {runtime}")
    ...
    if not projection_file_pass or not projection_contract_pass:
        raise RuntimeError(f"EX54 accepted EX48 data projection failed: file={projection_file_pass} checks={projection_checks}")
    ...
    if not data124["passed"] or not data1b["passed"]:
        raise RuntimeError(f"EX54 data preflight failed: 124m={data124['checks']} 1b={data1b['checks']}")
    free = shutil.disk_usage(run_dir).free
    disk_pass = free >= int(contract["checkpoint_retention"]["minimum_free_disk_bytes"])
    if not disk_pass:
        raise RuntimeError(f"EX54 requires 600 GB free, observed {free}")
    ...
    audit_code = "import json; from moonlight_optimizer import run_small_matrix_reference_audit; print(json.dumps(run_small_matrix_reference_audit('cuda'),sort_keys=True))"
    env = worker_env(snapshot, args.official_repo, args.gpus[0], contract["tuning"]["124m"]["cells"][1], P.sha256_file(contract_path), "preflight_small_audit")
    completed = subprocess.run([str(args.training_python.absolute()), "-c", audit_code], env=env, text=True, capture_output=True)
    if completed.returncode:
        raise RuntimeError(f"EX54 Moonlight reference audit failed: {completed.stdout}\n{completed.stderr}")
    ...
    if not payload["passed"]:
        raise RuntimeError(f"EX54 preflight failed: {checks}")
    P.atomic_json(run_dir / "status.json", {"status": "preflight_passed", "updated_at": now_iso()})
    print(f"EX54 preflight passed. Artifacts: {run_dir}")
...
def validate_local_summary(
    ...
    *,
    ...
    if not all(checks.values()):
        raise RuntimeError(f"invalid EX54 local summary {path}: {checks}")
    return summary
...
def run_memory_canary(
    ...
    raise RuntimeError("EX54 Moonlight does not use an optimizer-specific memory-canary stage; this function must not be called")
...
def run_tuning_cell(args: argparse.Namespace, snapshot: Path, contract: dict[str, Any], scale: str, cell: dict[str, Any], gpu: int) -> dict[str, Any]:
    ...
    if rc:
        raise RuntimeError(f"EX54 tuning cell failed {scale}/{cell['id']} rc={rc}")
    summary = validate_local_summary(
        attempt / "summary.json",
        scale=scale,
        seed=seed,
    ...
    payload = {
        "schema_version": "ex54_tuning_cell_v1", "passed": True, "scale": scale,
...
def schedule(items: list[Any], gpus: list[int], fn: Callable[[Any, int], Any]) -> list[Any]:
    """Load-balance independent single-GPU jobs while guaranteeing GPU coverage.

    Each physical GPU owns exactly one worker thread, so two scientific jobs
    can never overlap on the same device.  The first scheduling wave assigns
    one job to every available GPU; remaining heterogeneous 124M/1B work is
    pulled from a shared queue.  Thus tuning/formal parallelism changes only
    wall-clock scheduling, never per-run batch, seed, data, or optimizer state.
    """
    if not gpus:
        raise ValueError("EX54 scheduler requires at least one GPU")
    indexed = list(enumerate(items))
    if not indexed:
        return []
    workers = min(len(gpus), len(indexed))
    initial = indexed[:workers]
    queue: Queue[tuple[int, Any]] = Queue()
    for row in indexed[workers:]:
        queue.put(row)
    barrier = Barrier(workers) if workers > 1 else None

    def execute(gpu: int, row: tuple[int, Any]) -> tuple[int, Any]:
        index, item = row
        print(f"EX54_SCHED_START gpu={gpu} index={index} item={item}", flush=True)
        result = fn(item, gpu)
        if isinstance(result, dict):
            result = dict(result)
            # Preserve the physical GPU recorded by an already-completed
            # attempt. On resume, a different scheduler thread may only be
            # validating/reusing that artifact; it did not produce it.
            result.setdefault("physical_gpu", int(gpu))
        print(f"EX54_SCHED_DONE gpu={gpu} index={index} item={item}", flush=True)
        return index, result

    def worker(gpu: int, first: tuple[int, Any]) -> list[tuple[int, Any]]:
        local: list[tuple[int, Any]] = []
        if barrier is not None:
            barrier.wait()
        local.append(execute(gpu, first))
        while True:
            try:
    ...
    with ThreadPoolExecutor(max_workers=workers) as executor:
        pending = [
            executor.submit(worker, gpus[index], initial[index])
...
def tuning_selection_scientific_view(value: Any) -> Any:
    """Remove only execution placement from a frozen tuning selection.

    ``physical_gpu`` is provenance about where a completed attempt ran. It is
    not an input to the LR winner rule. All losses, cells, hashes, seeds, steps,
    and winner fields remain under exact resume comparison.
    """

    if isinstance(value, dict):
        return {
            key: tuning_selection_scientific_view(item)
            for key, item in value.items()
            if key != "physical_gpu"
        }
    if isinstance(value, list):
        return [tuning_selection_scientific_view(item) for item in value]
    return value
...
def select_cell(contract: dict[str, Any], scale: str, rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if set(by_id) != {cell["id"] for cell in spec["cells"]}:
        raise RuntimeError(f"incomplete EX54 tuning grid for {scale}")
...
def write_selected_contract(run_dir: Path, snapshot: Path, contract: dict[str, Any], selection_sha: str, selected: dict[str, Any]) -> Path:
    ...
    if path.is_file():
        ...
        if not all(stable_checks.values()):
            raise RuntimeError(
                "EX54 selected contract changed across resume: "
                f"{stable_checks}"
            )
        return path
...
def long_worker_profile(contract: dict[str, Any]) -> dict[str, Any]:
    """Adapt EX54's scale-indexed audit profile to the EX48 worker contract."""
...
LONG_WORKER_PROFILE_COMPAT_MARKER = "EX54_LONG_WORKER_CONTRACT_COMPAT_V3_SOURCE_VIEW"
LONG_WORKER_PROFILE_COMPAT_V2_MARKER = "EX54_LONG_WORKER_CONTRACT_COMPAT_V2"


def _v3_worker_contract_view_block(indent: str = "    ") -> str:
    # Kept as source text so frozen snapshots remain immutable. The generated
    # sibling adapter changes only the in-memory view consumed by the accepted
    # EX48 long worker.
    lines = [
        f"{indent}# {LONG_WORKER_PROFILE_COMPAT_MARKER}",
        f"{indent}original_read_json = protocol.read_json",
        f"{indent}contract_index = sys.argv.index('--contract')",
        f"{indent}contract_path = Path(sys.argv[contract_index + 1]).resolve()",
        f"{indent}base_index = sys.argv.index('--base-trainer')",
        f"{indent}base_trainer_path = Path(sys.argv[base_index + 1]).resolve()",
        f"{indent}base_trainer_sha256 = protocol.sha256_file(base_trainer_path)",
        "",
        f"{indent}def read_json_with_worker_profile(path: Path):",
        f"{indent}    payload = original_read_json(path)",
        f"{indent}    if Path(path).resolve() == contract_path:",
        f"{indent}        payload = dict(payload)",
        f"{indent}        profile = dict(payload['profiles']['1b'])",
        f"{indent}        profile.update(payload.get('profile', {{}}))",
        f"{indent}        profile.update({{",
        f"{indent}            'n_layer': 18, 'n_head': 16, 'n_embd': 2048,",
        f"{indent}            'intermediate_size': 5504,",
        f"{indent}            'expected_preconditioner_groups': {{'moonlight': 0}},",
        f"{indent}        }})",
        f"{indent}        payload['profile'] = profile",
        f"{indent}        grid = dict(payload.get('grid') or {{}})",
        f"{indent}        grid.update({{",
        f"{indent}            'methods': ['moonlight'],",
        f"{indent}            'seeds': list(payload['formal']['seeds']),",
        f"{indent}            'formal_units': len(payload['formal']['seeds']),",
        f"{indent}            'host_count': 1,",
        f"{indent}            'gpus': len(payload['execution']['physical_gpus']),",
        f"{indent}        }})",
        f"{indent}        payload['grid'] = grid",
        f"{indent}        accepted_sources = dict(payload.get('accepted_sources') or {{}})",
        f"{indent}        accepted_sources['scripts/17_llama_swiglu_validation/train_llama_swiglu.py'] = base_trainer_sha256",
        f"{indent}        payload['accepted_sources'] = accepted_sources",
        f"{indent}    return payload",
        "",
        f"{indent}protocol.read_json = read_json_with_worker_profile",
    ]
    return "\n".join(lines)


def patch_long_worker_profile_adapter(source: str) -> str:
    """Supply the EX48 worker compatibility view without changing contract bytes.

    V3 additionally binds the EX48 worker's legacy accepted_sources trainer key
    to the actual derived Moonlight --base-trainer hash. EX54 stores immutable
    parent provenance under parent/... keys, while the accepted EX48 worker
    validates the file it is directly asked to import using the legacy key.
    """
    ...
    if LONG_WORKER_PROFILE_COMPAT_MARKER in source:
        ...
        if missing:
            raise RuntimeError(
                "EX54 built-in V3 long-worker compatibility adapter is incomplete: "
                f"{missing}"
            )
        return source

    # V2 snapshots already contain profile/grid adaptation. Replace that whole
    # wrapper deterministically with V3 rather than mutating the frozen file.
    if LONG_WORKER_PROFILE_COMPAT_V2_MARKER in source:
        v2_start = source.index(
            f"    # {LONG_WORKER_PROFILE_COMPAT_V2_MARKER}\n"
        )
        v2_end_marker = "    protocol.read_json = read_json_with_worker_profile\n"
        v2_end = source.index(v2_end_marker, v2_start) + len(v2_end_marker)
        replacement = _v3_worker_contract_view_block() + "\n"
        patched = source[:v2_start] + replacement + source[v2_end:]
        compile(patched, "<ex54_long_worker_contract_compat_v3>", "exec")
        return patched

    # Older snapshots without an adapter receive the complete V3 wrapper.
    old = (
        "    module.P = protocol\n"
    ...
    new = (
        "    module.P = protocol\n"
        + _v3_worker_contract_view_block()
        + "\n    return int(module.main())"
    )
    if source.count(old) != 1:
        raise RuntimeError("EX54 long-worker V3 adapter anchor changed")
...
def runtime_long_worker_adapter(args: argparse.Namespace, snapshot: Path) -> Path:
    ...
    target = original
    if patched != source:
        target = snapshot / PACKAGE_REL / "long_worker_contract_compat_v3.py"
        if target.is_file():
            if target.read_text(encoding="utf-8") != patched:
                raise RuntimeError("EX54 long-worker V3 adapter changed across resume")
        else:
            P.atomic_text(target, patched)
    receipt_path = args.run_dir / "long_worker_contract_compatibility_amendment_v3.json"
    derived_trainer = snapshot / "derived/train_llama_moonlight.py"
    payload = {
        "schema_version": "ex54_long_worker_contract_compatibility_amendment_v3",
        "passed": True,
        "created_at": now_iso(),
        "reason": "supply_ex48_profile_grid_and_actual_derived_trainer_source_binding_in_memory",
        ...
        "injected_worker_view_keys": ["profile", "grid", "accepted_sources"],
        "accepted_ex48_legacy_trainer_key": "scripts/17_llama_swiglu_validation/train_llama_swiglu.py",
        "actual_base_trainer_path": str(derived_trainer),
        "actual_base_trainer_sha256": P.sha256_file(derived_trainer),
    ...
    if receipt_path.is_file():
        ...
        if any(existing.get(key) != payload[key] for key in stable):
            raise RuntimeError("EX54 long-worker V3 contract compatibility receipt changed")
    else:
...
def runtime_direct_trainer(args: argparse.Namespace, snapshot: Path) -> Path:
    ...
    Older EX54 snapshots were frozen before the CPU-ByteTensor normalization
    ...
    if patched != source:
        ...
        if target.is_file():
            if target.read_text(encoding="utf-8") != patched:
                raise RuntimeError("EX54 RNG compatibility source changed across resume")
        else:
            P.atomic_text(target, patched)
    receipt_path = args.run_dir / "rng_resume_compatibility_amendment_v1.json"
    payload = {
        "schema_version": "ex54_rng_resume_compatibility_amendment_v1",
        "passed": True,
        "created_at": now_iso(),
    ...
    if receipt_path.is_file():
        ...
        stable = set(payload) - {"created_at"}
        if any(existing.get(key) != payload[key] for key in stable):
            raise RuntimeError("EX54 RNG compatibility amendment no longer matches this run")
    else:
        P.atomic_json(receipt_path, payload)
    return target
    ...
    try:
        ...
        for name in ("1b_base", "1b_branch"):
            if not phase_valid(
                ...
                selected_contract_sha, 5402, data1b_sha,
...
def run_engineering_pilots(args: argparse.Namespace, snapshot: Path, contract: dict[str, Any], selected: dict[str, Any], selection_sha: str, selected_contract: Path) -> dict[str, Any]:
    ...
    for target in (2, 4):
        ...
        if rc:
            raise RuntimeError(f"EX54 124M exact-resume pilot failed target={target} rc={rc}")
    ...
    if checkpoint124.is_file():
        ...
        raise RuntimeError("EX54 124M pilot checkpoint disappeared without a certificate")
    ...
    if not phase_valid(
        ...
        if rc:
            raise RuntimeError(f"EX54 1B in-place resume pilot failed rc={rc}")
    ...
    if not phase_valid(
        ...
        raise RuntimeError("EX54 1B resumed pilot base failed state/lineage/hash audit")
    if int(P.read_json(base / "summary.json").get("resume_count", 0)) < 1:
        raise RuntimeError("EX54 1B pilot did not record the planned in-place resume")
    branch = args.run_dir / "tuning/engineering_pilot/1b_branch"
    if not phase_valid(
        branch, contract_sha, 5402, data1b_sha,
        selection_sha=selection_sha, contract=contract,
        full_checkpoint_hash=True,
    ):
        if (branch / "phase_manifest.json").is_file():
            raise RuntimeError("EX54 1B pilot branch manifest exists but failed audit")
        source_path = Path(base_manifest["checkpoint"]["path"])
        if not source_path.is_file():
            raise RuntimeError("EX54 1B pilot base was retired before its branch passed")
        rc = run_logged(
            long_worker_command(
                args, snapshot, selected_contract, "pilot_branch_4", branch,
                seed=5402, source=base_manifest["checkpoint"],
            ),
            env,
            branch / "worker.log",
        )
        if rc:
            raise RuntimeError(f"EX54 1B branch pilot failed rc={rc}")
    if not phase_valid(
        branch, contract_sha, 5402, data1b_sha,
        selection_sha=selection_sha, contract=contract,
        full_checkpoint_hash=True,
    ):
        raise RuntimeError("EX54 1B pilot branch failed state/lineage/hash audit")
    if P.read_json(branch / "summary.json").get("source_checkpoint_sha256") != base_manifest["checkpoint"]["sha256"]:
        raise RuntimeError("EX54 1B pilot branch did not originate at the certified fork")
    retired = []
    ...
    payload = {
        "schema_version": "ex54_engineering_pilot_v1", "passed": True,
        "quality_eligible": False, "timing_eligible": False,
    ...
    if not engineering_pilot_manifest_valid(
        ...
        raise RuntimeError("EX54 engineering pilot manifest failed independent replay")
    return payload
...
def tuning_manifest_valid(
    ...
    try:
        ...
        if manifest.get("passed") is not True:
            return False
        expected_tuning_seeds = {scale: int(contract["tuning"][scale]["seed"]) for scale in ("124m", "1b")}
        formal_seeds = {int(seed) for seed in contract["formal"]["seeds"]}
        if (
            manifest.get("tuning_seeds") != expected_tuning_seeds
            or not set(expected_tuning_seeds.values()).isdisjoint(formal_seeds)
            or manifest.get("formal_seed_overlap") is not False
            or manifest.get("tuning_gpus_used") != [0, 1]
            or set(int(value) for value in manifest.get("gpu_assignment", {}).values()) != {0, 1}
            or manifest.get("parallel_single_gpu_jobs") is not True
            or manifest.get("compile_cache_policy") != "per_physical_gpu"
        ):
            return False
        ...
        profile_receipt = (
            args.run_dir / "long_worker_contract_compatibility_amendment_v3.json"
        )
        if not (
...
def tuning(args: argparse.Namespace) -> None:
    ...
    if pre.get("passed") is not True:
        raise RuntimeError("EX54 tuning requires passed preflight")
    ...
    if final_manifest.is_file() and tuning_manifest_valid(args, snapshot, contract):
        print(f"skip passed EX54 tuning: {args.run_dir}")
        return
    tuning_seeds = {int(contract["tuning"][scale]["seed"]) for scale in ("124m", "1b")}
    formal_seeds = {int(seed) for seed in contract["formal"]["seeds"]}
    if not tuning_seeds.isdisjoint(formal_seeds):
        raise RuntimeError(
            f"EX54 fairness violation: tuning seeds {sorted(tuning_seeds)} overlap formal seeds {sorted(formal_seeds)}"
        )

    # One shared queue across both scales keeps GPU0/GPU1 occupied. Put 1B
    # cells first because they are longer; 124M cells naturally fill gaps.
    tuning_tasks = [
        (scale, cell)
        for scale in ("1b", "124m")
        for cell in contract["tuning"][scale]["cells"]
    ]
    tuning_rows = schedule(
        tuning_tasks,
        args.gpus,
        lambda item, gpu: run_tuning_cell(
            args, snapshot, contract, item[0], item[1], gpu
        ),
    )
    tuning_gpus = sorted({int(row["physical_gpu"]) for row in tuning_rows})
    if tuning_gpus != args.gpus:
        raise RuntimeError(
            f"EX54 tuning must exercise both physical GPUs {args.gpus}; observed {tuning_gpus}"
        )
    selected: dict[str, Any] = {}
    for scale in ("124m", "1b"):
        rows = [row for row in tuning_rows if row["scale"] == scale]
        selected[scale] = select_cell(contract, scale, rows)
    selection_path = args.run_dir / "tuning/selection.json"
    selection_payload = {
        "schema_version": "ex54_scale_specific_selection_v1", "created_at": now_iso(),
    ...
    if selection_path.is_file():
        ...
        stable_checks = {
            ...
            "scales": tuning_selection_scientific_view(
                existing_selection.get("scales")
            ) == tuning_selection_scientific_view(selected),
        }
        if not all(stable_checks.values()):
            raise RuntimeError(
                "EX54 frozen tuning selection changed across resume: "
                f"{stable_checks}"
            )
        # Freeze the exact original selection payload for every downstream
        # manifest. A replay may differ only in physical_gpu placement.
        if existing_selection.get("scales") != selected:
            amendment_path = (
                args.run_dir
                / "tuning/selection_resume_execution_metadata_amendment_v1.json"
            )
            amendment = {
                "schema_version": "ex54_tuning_selection_resume_execution_metadata_v1",
                "passed": True,
                "reason": "physical_gpu_only_resume_replay_difference",
                "outcome_based_change": False,
                "scientific_selection_changed": False,
                "winner_rule_changed": False,
                "ignored_execution_fields": ["physical_gpu"],
                "selection_sha256": P.sha256_file(selection_path),
                "existing_scientific_sha256": P.canonical_sha256(
                    tuning_selection_scientific_view(
                        existing_selection.get("scales")
                    )
                ),
                "recomputed_scientific_sha256": P.canonical_sha256(
                    tuning_selection_scientific_view(selected)
                ),
            }
            if amendment_path.is_file():
                previous = P.read_json(amendment_path)
                checks = {
                ...
                if not all(checks.values()):
                    raise RuntimeError(
                        "EX54 tuning resume amendment changed: "
                        f"{checks}"
                    )
            else:
                P.atomic_json(amendment_path, amendment)
            print(
                "EX54_SELECTION_RESUME_STABLE="
                "physical_gpu_only_difference_ignored",
                flush=True,
            )
        selected = existing_selection["scales"]
    else:
    ...
    payload = {
        "schema_version": "ex54_tuning_manifest_v1", "passed": pilot.get("passed") is True,
        ...
        "selected": selected,
        "tuning_seeds": {scale: int(contract["tuning"][scale]["seed"]) for scale in ("124m", "1b")},
        "formal_seeds": sorted(formal_seeds),
        "formal_seed_overlap": False,
        "tuning_gpus_used": tuning_gpus,
        "gpu_assignment": {f"{row['scale']}/{row['cell']['id']}": int(row["physical_gpu"]) for row in tuning_rows},
        "parallel_single_gpu_jobs": True,
        "compile_cache_policy": contract["execution"]["compile_cache_policy"],
        "engineering_pilot_sha256": P.sha256_file(args.run_dir / "tuning/engineering_pilot_manifest.json"),
    ...
    print(f"EX54 tuning passed. Artifacts: {args.run_dir}")
...
def local_unit_valid(
    ...
    try:
        ...
        return (
            ...
            and int(manifest.get("physical_gpu", -1)) in {int(value) for value in contract["execution"]["physical_gpus"]}
...
def run_formal_124(args: argparse.Namespace, snapshot: Path, contract: dict[str, Any], selection: dict[str, Any], selection_sha: str, selected_contract: Path, seed: int, gpu: int) -> dict[str, Any]:
    ...
    if rc:
        raise RuntimeError(f"EX54 formal 124M seed{seed} failed rc={rc}")
    summary = validate_local_summary(
        unit / "summary.json",
        scale="124m",
        seed=seed,
...
def complete_checkpoint_retirement(
    ...
    if retirement.is_file():
        ...
        if not all(checks.values()):
            raise RuntimeError(f"changed EX54 retirement certificate: {checks}")
    else:
        if not path.is_file():
            raise RuntimeError(
                f"EX54 checkpoint disappeared before its retirement certificate: {path}"
            )
        if (
            path.stat().st_size != int(checkpoint["bytes"])
            or P.sha256_file(path) != checkpoint["sha256"]
        ):
            raise RuntimeError(f"cannot retire changed EX54 checkpoint: {path}")
        payload = {
            "schema_version": "ex54_checkpoint_retirement_v1",
            "passed": True,
            "path": str(path),
    ...
    if path.is_file():
        if (
            ...
            raise RuntimeError(f"changed EX54 checkpoint after certification: {path}")
        path.unlink()
    return payload
...
def non10b_unit_valid(
    ...
    try:
        ...
        # EX54 has no continuation experiment. Both fork-only checkpoints are
    ...
    *,
    ...
    for phase in phases:
        ...
        if not phase_valid(
            ...
            if rc or not phase_valid(
                ...
                raise RuntimeError(f"EX54 formal 1B seed{seed}/{phase['id']} failed rc={rc}")
    ...
    for phase in endpoint_phases:
        ...
        if not checkpoint.is_file() or P.sha256_file(checkpoint) != manifest["checkpoint"]["sha256"]:
            raise RuntimeError(f"EX54 retained endpoint hash failed: {checkpoint}")
        endpoints[phase["budget_id"]] = manifest["checkpoint"]
    payload = {
        "schema_version": P.UNIT_MANIFEST_SCHEMA if finalize else "ex54_non10b_unit_v1",
        "passed": True,
...
def ensure_controller_amendment(args: argparse.Namespace) -> dict[str, Any] | None:
    # Fresh Moonlight EX54 runs are self-contained. There is no cross-experiment
    # handoff/controller amendment path.
    return None

def require_10b_runtime_admission(args: argparse.Namespace) -> dict[str, Any]:
    raise RuntimeError("EX54 Moonlight has no 10B continuation; Experiment 57 is independent")
...
def formal_non10b(args: argparse.Namespace) -> None:
    ...
    # Long 1B units enter the shared two-GPU queue first; shorter 124M units
    # backfill whichever GPU becomes free. This minimizes idle time without DDP.
    tasks = [("1b", int(seed)) for seed in contract["formal"]["seeds"]] + [
        ("124m", int(seed)) for seed in contract["formal"]["seeds"]
    ]
    ...
    if formal_gpus != args.gpus:
        raise RuntimeError(
            f"EX54 formal must exercise both physical GPUs {args.gpus}; observed {formal_gpus}"
        )
    payload = {
        "schema_version": "ex54_moonlight_formal_manifest_v1",
        ...
        "gpu_assignment": {f"{row['scale']}/seed{row['seed']}": int(row["physical_gpu"]) for row in rows},
        "parallel_single_gpu_jobs": True,
        "compile_cache_policy": contract["execution"]["compile_cache_policy"],
        "ddp": False,
    ...
    # reusable, but this is EX54's final scientific boundary, not a handoff.
    P.atomic_json(args.run_dir / "formal/formal_non10b_manifest.json", payload)
    if not payload["passed"]:
        raise RuntimeError("EX54 Moonlight formal stage incomplete")
    P.atomic_json(args.run_dir / "status.json", {"status": "formal_passed", "updated_at": now_iso()})
    print(f"EX54 Moonlight formal stage passed. Artifacts: {args.run_dir}")

def formal_10b(args: argparse.Namespace) -> None:
    raise RuntimeError("EX54 Moonlight ends at 6.97B; 10B is outside this experiment scope")
...
def verify(args: argparse.Namespace) -> None:
    ...
    if not formal_path.is_file() or P.read_json(formal_path).get("passed") is not True:
        raise RuntimeError("EX54 verify requires a passed formal stage")
    ...
    if completed.returncode:
        raise RuntimeError(f"EX54 analysis failed: {completed.stdout}\n{completed.stderr}")
    ...
    if audit.returncode:
        raise RuntimeError(f"EX54 verification failed: {audit.stdout}\n{audit.stderr}")
    ...
    if not (verification.get("passed") is True and verification.get("full_checkpoint_hash") is True):
        raise RuntimeError("EX54 full native verification receipt is missing")
    payload = {
        "schema_version": "ex54_moonlight_completion_v1",
        "status": "completed",
    ...
    print(f"EX54 Moonlight suite completed. Artifacts: {args.run_dir}")
...
def upload(args: argparse.Namespace) -> None:
    ...
    if completed.returncode:
        raise RuntimeError(f"EX54 W&B upload failed rc={completed.returncode}")
...
def check(args: argparse.Namespace) -> None:
    ...
    payload["plan"] = {
        ...
        "tuning_formal_seed_overlap": False,
        "tuning_parallel_workers": 2,
        "formal_parallel_workers": 2,
        "compile_cache_policy": "per_physical_gpu",
...
if __name__ == "__main__":
    try:
        ...
        print(f"EX54 stopped cleanly: {type(exc).__name__}: {exc}", file=sys.stderr)
        raise SystemExit(2)
