#!/usr/bin/env python3
"""Pure-stdlib contract, data, cursor, and artifact helpers for experiment 56."""
...
SCHEMA = "ex56_llama1b_10b_global_diag_formal_v1"
CHECKPOINT_SCHEMA = "ex56_llama1b_global_diag_segment_checkpoint_v1"
PHASE_MANIFEST_SCHEMA = "ex56_llama1b_global_diag_phase_manifest_v1"
UNIT_MANIFEST_SCHEMA = "ex56_llama1b_global_diag_unit_manifest_v1"
DATA_AUDIT_SCHEMA = "ex56_llama1b_global_diag_data_audit_v1"
METRIC_FIELDS = (
    "event",
    "phase_id",
    "schedule",
...
def content_inventory_sha256(identity: dict[str, list[dict[str, Any]]]) -> str:
    """Hash stable content-defining fields from an audited shard inventory."""
    fields = (
        "name", "index", "tokens", "consumable_tokens", "bytes",
        "header_sha256", "sha256",
    )
    rows = [
        {"split": split, **{field: row[field] for field in fields}}
        for split in ("train", "validation")
        for row in identity[split]
    ]
...
def validate_contract(contract: dict[str, Any]) -> dict[str, bool]:
    ...
    return {
        ...
        "experiment": int(contract.get("experiment_number", -1)) == 56,
        "formal_authorized": contract.get("status")
        == "preregistered_formal_authorized_by_user_20260817",
        "grid": contract["grid"]["methods"]
        == ["global_diag"]
        and contract["grid"]["seeds"] == [2024, 2025, 2026]
        and int(contract["grid"]["formal_units"]) == 3
        and int(contract["grid"]["host_count"]) == 1
        and int(contract["grid"]["gpus"]) == 1
        and contract["grid"]["physical_gpus"] == [3]
        and int(contract["grid"]["maximum_concurrent_training_processes"]) == 1,
        ...
        "accepted_ex48_data": contract["data"]["accepted_ex48_inventory_sha256"]
        == "76848d39697aa6e0a7083e4445185fd7215b4f7ddad411d60a7cd6943c384b21"
        and contract["data"]["accepted_ex48_content_projection_sha256"]
        == "2820057bcdfa76afd9523c612c7e6846b3c5f545c98066c129c8b9d4c06a9b10",
        "runtime_kernel": contract["runtime"]["accepted_triton_kernels_sha256"]
        == "b51ac50c699b05306619d92cb9ec6edadd266d8118c53f5b9726db76480ea16d",
        "runtime_gpu_geometry": int(contract["runtime"]["gpu_count"]) == 1
        and contract["runtime"]["nvidia_driver"] == "580.95.05",
        "global_diag_profile": int(
            contract["profile"]["expected_preconditioner_groups"]["global_diag"]
        ) == 72
        and int(contract["profile"]["expected_global_diag_k_state_bytes"]) == 1677312,
        "accepted_ex48_initialization": contract["accepted_ex48_initialization_sha256"]
        == {
            "2024": "a9d4dd25e7d7992e4c6f922a0b688b8398f15ee04645a8b90ebf5a9954e44024",
            "2025": "2143f088e467eeeb03311f17856e5d2fd88274a2f725adfcccdd79861221ea8e",
            "2026": "521d734608ff08ab6a191c4c1a41412c40bf53985ee3cbcefc3ab15457d39fec",
        },
        "fresh_from_init": contract["lr_policy"]["from_scratch_required"] is True
        and contract["lr_policy"]["ex52_checkpoint_resume_forbidden"] is True,
...
def assert_contract(contract: dict[str, Any]) -> None:
    ...
    if not all(checks.values()):
        raise RuntimeError(f"experiment 56 contract failed: {checks}")
...
def audit_data_dir(data_dir: Path, contract: dict[str, Any], full_hash: bool = True) -> dict[str, Any]:
    ...
    content_projection_sha256 = content_inventory_sha256(identity)
    return {
