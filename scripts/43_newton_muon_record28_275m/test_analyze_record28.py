#!/usr/bin/env python3
"""Unit tests for the experiment-43 independent analyzer."""
...
SPEC = importlib.util.spec_from_file_location(
    "analyze_record28", HERE / "analyze_record28.py"
)
...
class Record28AnalyzerTests(unittest.TestCase):
    def setUp(self) -> None:
        self.temporary = tempfile.TemporaryDirectory()
        self.root = Path(self.temporary.name)
        self.run = self.root / "run"
        self.contract = self.root / "record28_contract.json"
        write_json(
            self.contract,
            {
                "schema_version": 1,
            ...
            for method in A.EXPECTED_METHODS:
                ...
                write_json(
                    command_path,
                    {
                        ...
                        "environment": {
                            "RECORD28_METHOD": method,
                            "RECORD28_CPROJ_K_MODE": cproj_mode,
                        },
                    },
                )
                manifest = {
    ...
    def test_complete_analysis_and_primary_delta(self) -> None:
        ...
        self.assertEqual(manifest["accepted_formal_cells"], 16)
        self.assertEqual(len(manifest["accepted_cell_fingerprints"]), 16)
        self.assertEqual(
        ...
        reusable, checks = S.validate_reusable_analysis(
            output / "record28_analysis_manifest.json", self.run
        )
    ...
    def test_command_environment_mismatch_is_rejected(self) -> None:
        ...
        payload["environment"]["RECORD28_CPROJ_K_MODE"] = "block4"
    ...
    def test_t_summary_uses_df3_student_t(self) -> None:
        summary = A.t_summary([-0.004, -0.003, -0.002, -0.001])
        self.assertEqual(summary["n_seeds"], 4)
        expected_sd = math.sqrt(5.0 / 3.0) * 0.001
        self.assertAlmostEqual(summary["sample_sd"], expected_sd)
        self.assertAlmostEqual(
            summary["ci95_high_t_df3"] - summary["mean"],
            A.T_CRITICAL_95_DF3 * expected_sd / 2,
        )

    def test_checked_in_contract_matches_frozen_analysis(self) -> None:
        checked_in = json.loads(
            (HERE / "record28_contract.json").read_text(encoding="utf-8")
        )
    ...
    def test_corrupt_committed_analysis_selects_fresh_retry(self) -> None:
        ...
        committed = output / "record28_analysis_manifest.json"
        committed_before = committed.read_bytes()
        report = output / "RECORD28_ANALYSIS_REPORT.md"
        report.write_text(
