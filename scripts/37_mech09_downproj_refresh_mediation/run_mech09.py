#!/usr/bin/env python3
"""Certify, smoke-test, schedule, resume, and analyze MECH-09."""
...
WORKER_VERSION = "2026-07-28.1"
ANALYSIS_VERSION = "2026-07-28.1"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--contract",
        default=HERE / "refresh_mediation_contract.json",
        type=Path,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--execution-domain", default="llama-host-llama1b-mech09"
    )
...
def reuse_mech08_hash_certificate(
    spec: dict[str, Any],
    mech08_run: Path,
    destination: Path,
) -> dict[str, Any] | None:
    source = mech08_run / "checkpoint_hashes" / f"{spec['cell']}.json"
    ...
    stat = path.stat()
    checks = {
        "source_passed": payload.get("passed") is True,
        "cell": payload.get("cell") == spec["cell"],
        "path": payload.get("path") == spec["path"],
        "size": int(payload.get("bytes", -1)) == stat.st_size,
        "mtime": int(payload.get("mtime_ns", -1)) == stat.st_mtime_ns,
    ...
    if not all(checks.values()):
        return None
    reused = dict(payload)
    reused["reused_from_mech08"] = str(source)
    reused["reuse_checks"] = checks
    write_json(destination, reused)
    return reused


def audit_control_reference(
...
def completed_manifest(
    ...
    *,
    ...
    return (
        ...
        and manifest.get("intervention") == intervention
...
def worker_command(
    ...
    command = [
        ...
        str(args.contract),
        "--mech08-control-reference",
        str(args.mech08_control_reference),
        "--train-data-pattern",
    ...
    with command_log.open("a", encoding="utf-8") as log:
        while pending or running:
            while pending and available and not failed:
                ...
                log.flush()
                print(
                    f"MECH-09 launch gpu={gpu} label={job['label']}",
                    flush=True,
                )
            ...
            for job in running:
                ...
                if return_code != 0:
                    ...
                    print(
                        f"MECH-09 passed gpu={job['gpu']} "
                        f"label={job['label']}",
                        flush=True,
                    )
            running = still_running
    if failed:
        raise RuntimeError(f"MECH-09 worker failures: {failed}")
    ...
    for cell in config["origins"]:
        ...
        for intervention in config["interventions"]:
            for replica in config["data_replicas"]:
                ...
                manifest = output / "mech09_manifest.json"
                if completed_manifest(
                    ...
                    print(
                        "MECH-09 resume: already passed "
                        f"{tier}/{cell}/{intervention}/replica_{replica}",
                        flush=True,
                    )
                    continue
                ...
                jobs.append(
                    {
                        "label": (
                            f"{tier}/{cell}/{intervention}/replica_{int(replica)}"
                        ),
                        "output": output,
...
def main() -> None:
    ...
    try:
        ...
        expected_formal_jobs = (
            ...
            * len(contract["formal"]["interventions"])
        ...
        preflight_checks = {
            ...
            "formal_jobs": expected_formal_jobs == 24,
        ...
        if not all(preflight_checks.values()):
            raise RuntimeError(f"MECH-09 preflight failed: {preflight_checks}")

        control_audit = audit_control_reference(
            reference, args.mech08_run_dir.resolve()
        )
        write_json(run / "mech08_control_preflight.json", control_audit)
        if not control_audit["passed"]:
            raise RuntimeError("MECH-08 control preflight failed")
        ...
        for spec in contract["checkpoints"]:
            ...
            if reused is None:
                reused = reuse_mech08_hash_certificate(
                    spec,
        ...
        smoke_payload = smoke_aggregate(run, contract, contract_sha)
        write_json(smoke_manifest, smoke_payload)
        if not smoke_payload["passed"]:
            raise RuntimeError(f"MECH-09 aggregate smoke failed: {smoke_payload}")
        ...
        analysis_manifest = run / "analysis" / "mech09_analysis_manifest.json"
        ...
        if analysis_manifest.is_file():
            ...
            analysis_complete = (
                ...
                and analysis_payload.get("contract_sha256") == contract_sha
                and analysis_payload.get("control_reference_sha256")
                == reference_sha
            )
        if not analysis_complete:
            analysis_command = [
                sys.executable,
                str(HERE / "analyze_mech09.py"),
                "--run-dir",
                str(run),
                "--contract",
                ...
                "--mech08-run-dir",
                str(args.mech08_run_dir.resolve()),
            ]
        ...
        raise
    print(f"MECH-09 artifacts: {run}")
    print(
        "MECH-09 manifest: "
        f"{run / 'analysis' / 'mech09_analysis_manifest.json'}"
    )
