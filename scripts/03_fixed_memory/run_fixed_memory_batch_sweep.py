...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run fixed-memory batch-size sweeps for testing whether K-state "
            "release can be converted into larger feasible batches."
        )
    )
    ...
    parser.add_argument(
        ...
        default=["muon", "newton", "release_all_cproj"],
    )
    parser.add_argument("--batch-sizes", type=int, nargs="+", default=[2, 4, 6, 8, 10, 12, 14, 16])
    ...
    parser.add_argument("--cuda-memory-budget-gib", type=float, default=None)
    ...
    parser.add_argument("--wandb-project", default="Selective-Newton-Muon-MainConf-FixedMemory")
    ...
    parser.add_argument("--run-prefix", default="mainconf_fixed_memory_batch_sweep")
...
def build_commands(args: argparse.Namespace, middle_mask: str | None, release_all_mask: str | None) -> list[list[str]]:
    commands: list[list[str]] = []
    tags = (
        f"publication,{args.dataset},fixed_memory_batch_sweep,"
        f"L{args.n_layer},D{args.n_embd}"
    )
        ...
        for batch_size in args.batch_sizes:
            ...
            for method in args.methods:
                if method in ("muon", "newton"):
                    ...
                    run_name = f"{args.run_prefix}_bs{batch_size}_{method_label}_seed{seed}"
                    ...
                    run_name = f"{args.run_prefix}_bs{batch_size}_{label}_seed{seed}"
                    ...
                    release_frac = args.release_all_cproj_frac if args.dry_run else read_mask_target(release_all_mask)
...
def run_sweep_commands(commands: list[list[str]], args: argparse.Namespace) -> None:
    ...
    for cmd in commands:
        ...
        try:
            ...
            if not args.keep_going:
                raise
    if failures:
        print(f"fixed-memory sweep completed with {failures} failed command(s)")
...
def main() -> None:
    ...
    run_sweep_commands(commands, args)
