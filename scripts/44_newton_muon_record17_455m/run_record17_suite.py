#!/usr/bin/env python3
"""Bootstrap, preflight, schedule, recover, and analyze experiment 44."""
...
import analyze_record17 as record17_analysis
import record17_common as common
from record17_source_builder import build_all_sources, self_test_diag_math
from run_record17_cell import runtime_probe


SCRIPT_VERSION = "2026-07-30.7"
SCRIPT_DIR = Path(__file__).resolve().parent
RECOVERY_COMMAND_RELATIVE = (
    Path("commands")
    / "44_newton_muon_record17_455m"
    / "20260730_newton_muon_record17_455m.sh"
)
SNAPSHOT_FILES = (
    "record17_common.py",
    "record17_source_builder.py",
    "run_record17_cell.py",
    "run_record17_suite.py",
    "analyze_record17.py",
    "test_analyze_record17.py",
    "test_record17.py",
    "record17_contract.json",
    "RECORD17_455M_CONTRACT.md",
    "README.md",
    "upstream/record17_train_gpt_medium.py",
)
...
def recovery_command_path() -> Path:
    ...
    raise FileNotFoundError(
        "experiment-44 recovery launcher is missing; checked: "
...
def parse_args() -> argparse.Namespace:
    ...
    if len(args.gpus) > 2:
        parser.error("experiment 44 supports at most two quality lanes")
...
def bootstrap_snapshot(args: argparse.Namespace) -> dict[str, Any]:
    ...
    record17_upstream = common.audit_vendored_record17(
        SCRIPT_DIR / "upstream" / "record17_train_gpt_medium.py"
    )
    common.atomic_write_json(
        args.run_dir / "preflight" / "official_repo.json", provenance
    )
    common.atomic_write_json(
        args.run_dir / "preflight" / "record17_upstream.json",
        record17_upstream,
    )
    ...
    if set(derived) != set(common.METHODS):
        raise RuntimeError(
            "derived Record-17 source inventory does not match the frozen "
            f"method set: observed={sorted(derived)}, "
            f"expected={sorted(common.METHODS)}"
        )
    ...
    if len({record["derived_sha256"] for record in source_records.values()}) != 1:
        raise RuntimeError(
            "all four methods must share one frozen environment-dispatched "
            "Record-17 source"
        )
    ...
    manifest = {
        ...
        "record17_upstream_provenance": record17_upstream,
...
def forwarded_arguments(args: argparse.Namespace, *, snapshot_active: bool) -> list[str]:
    command = [
        controller_python(),
        str(snapshot_controller(args.run_dir) / "run_record17_suite.py"),
        "--run-dir",
        str(args.run_dir),
        "--live-repo",
        str(args.live_repo),
...
def source_drift_annotation(args: argparse.Namespace) -> dict[str, Any]:
    records = []
    for name in SNAPSHOT_FILES:
        live = args.live_repo / "scripts" / "44_newton_muon_record17_455m" / name
...
def preflight(args: argparse.Namespace, snapshot: dict[str, Any]) -> dict[str, Any]:
    ...
    if live_incomplete_processes:
        raise RuntimeError(
            "an earlier experiment-44 training process is still alive; "
    ...
    if reusable_test_result:
        ...
        test_env["RECORD17_OFFICIAL_REPO"] = str(args.official_repo)
        ...
        if not controller_tests["passed"]:
            raise RuntimeError(
                f"experiment-44 controller tests failed; inspect {test_log}"
            )
    ...
    if len(selected_uuids) != len(args.gpus):
        raise RuntimeError(
            "multiple experiment-44 GPU lane arguments resolve to the same "
    ...
    if active:
        raise RuntimeError(f"selected experiment-44 GPU lanes are not idle: {active}")
    ...
    if disk.free < minimum_free:
        raise RuntimeError(
            f"insufficient free disk for 12 model-only checkpoints: "
    ...
    payload = {
        ...
        "contract_sha256": common.sha256_file(
            snapshot_controller(args.run_dir) / "record17_contract.json"
        ),
...
def expected_cell_values(
    ...
    *,
    ...
    protocol = {
        # Cross the first frozen Newton refresh at logical update 24 and
        # retain three post-refresh updates.
        "smoke": (27, 27 * 524_288),
        "formal": (5960, 3_124_756_480),
    }[stage]
    return {
        "stage": stage,
        "seed": seed,
...
def cell_command(
    ...
    *,
    ...
    command = [
        controller_python(),
        str(snapshot_controller(args.run_dir) / "run_record17_cell.py"),
        "--attempt-dir",
        str(attempt),
        "--stage",
        stage,
        ...
        "--contract",
        str(snapshot_controller(args.run_dir) / "record17_contract.json"),
...
def launch_jobs(
    ...
    while pending or active:
        ...
        if failures or INTERRUPTED_SIGNAL is not None:
            ...
            raise RuntimeError(f"experiment-44 worker failures: {failures}")
    ...
    *,
    ...
    if stage == "formal" and len(args.gpus) == 2:
        ...
        lane_balance_passed = all(
            ...
            and sum(counts.values()) == len(common.SEEDS)
...
def validate_reusable_analysis(
    ...
    """Validate both analysis outputs and the exact 12 accepted inputs."""
    ...
    try:
        ...
        current_fingerprints = (
            record17_analysis.collect_accepted_cell_fingerprints(run_dir)
        )
        current_digest = (
            record17_analysis.accepted_cells_fingerprint_sha256(
                current_fingerprints
            )
        )
...
def select_analysis_output(
    ...
    for output in candidates:
        manifest = output / "record17_analysis_manifest.json"
...
def run_analysis(args: argparse.Namespace) -> Path:
    ...
    command = [
        controller_python(),
        str(snapshot_controller(args.run_dir) / "analyze_record17.py"),
        "--run-dir",
        str(args.run_dir),
        "--contract",
        str(snapshot_controller(args.run_dir) / "record17_contract.json"),
    ...
    if payload is None:
        raise RuntimeError(
            f"experiment-44 analysis did not pass sealed validation: "
            f"{manifest}: {checks}"
        )
    return manifest
...
def dry_run_plan(
    ...
    plan = {
        ...
        "formal_updates": 5960,
        "tokens_per_update": 524_288,
        "formal_train_tokens": 3_124_756_480,
        "formal_validation_rows": 49,
        "gpus": args.gpus,
        "method_orders": {
    ...
    print(f"RECORD17_DRY_RUN_PLAN={args.run_dir / 'dry_run_plan.json'}")
...
def freeze_or_validate_suite_plan(
    ...
    proposed = {
        ...
        "experiment": 44,
        "recipe": (
            "paper-matched single-GPU adaptation of Modded-NanoGPT "
        ...
        "record17_upstream_provenance": snapshot[
            "record17_upstream_provenance"
        ],
        ...
        "tokens_per_update": 524_288,
        "formal_train_tokens": 3_124_756_480,
        "formal_validation_interval_updates": 125,
        "formal_validation_steps": [
            *range(0, 5876, 125),
            5960,
        ],
        "formal_validation_rows": 49,
...
def main_snapshot(args: argparse.Namespace) -> None:
    ...
    with common.exclusive_file_lock(
        ...
        common.atomic_write_json(
            args.run_dir / "record17_suite_manifest.json", final
        )
        common.atomic_write_json(
            args.run_dir / "status.json",
            {
                ...
                "passed": True,
                "manifest": str(
                    args.run_dir / "record17_suite_manifest.json"
                ),
                "completed_at": common.utc_now(),
            },
        )
        handoff_manifest = write_handoff_manifest(args.run_dir)
        print(f"RECORD17_ARTIFACTS={args.run_dir}")
        print(
            f"RECORD17_MANIFEST="
            f"{args.run_dir / 'record17_suite_manifest.json'}"
        )
        print(f"RECORD17_HANDOFF_MANIFEST={handoff_manifest}")
        if pending_wandb:
            print(
                "RECORD17_WANDB_PENDING=1; rerun the printed recovery command. "
