#!/usr/bin/env python3
"""Independent, paired-seed analysis for experiment 44 (Medium Record #17).
...
EXPECTED_TOTAL_STEPS = 5960
EXPECTED_TOKENS_PER_UPDATE = 524_288
EXPECTED_TRAIN_TOKENS = 3_124_756_480
EXPECTED_VALIDATION_STEPS = tuple(range(0, 5876, 125)) + (5960,)
COMMON_TARGET_LOSS = 2.95
PRACTICAL_MARGIN = 0.002
T_CRITICAL_95_DF2 = 4.302652729911275
FLOAT_TOLERANCE = 1e-10
INITIAL_VALIDATION_TOLERANCE = 1e-6
DEFINITIVE_PRACTICAL_CLASSIFICATIONS = frozenset(
    {
        "candidate_better_beyond_margin",
        "candidate_worse_beyond_margin",
        "quality_equivalent_within_margin",
    }
)
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Analyze the 12 accepted experiment-44 formal cells."
    )
...
def validate_contract(contract: dict[str, Any]) -> dict[str, Any]:
    if contract.get("schema_version") != 1:
        raise RuntimeError("record17 contract schema_version must be 1")
    seeds = tuple(
        int(value)
    ...
    if not all(checks.values()):
        raise RuntimeError(f"frozen experiment-44 contract mismatch: {checks}")
    return {
        "checks": checks,
        "seeds": seeds,
        "methods": methods,
...
def collect_accepted_cell_fingerprints(
    ...
    """Return the canonical 12-cell input identity used by an analysis.
    ...
    if len(rows) != 12:
        raise RuntimeError(f"expected 12 accepted fingerprints, got {len(rows)}")
    return rows
...
def validate_summary(
    ...
    *,
    ...
    peak_memory_scope = summary.get("peak_memory_scope")
    if peak_memory_scope != "counted_run_after_warmup_reset_including_validation":
        raise RuntimeError(
            f"{label}:peak_memory_scope mismatch: {peak_memory_scope!r}"
        )
    state: dict[str, Any] = {
...
def load_cell(
    ...
    command_environment_checks = {
        "method": command_environment.get("RECORD17_METHOD") == method,
        "cproj_k_mode": (
            command_environment.get("RECORD17_CPROJ_K_MODE")
            == expected_cproj_mode
        ),
    }
...
def validate_pairing(
    ...
    checks: dict[str, Any] = {
        "formal_cell_coverage_12_of_12": set(by_key) == expected and len(rows) == 12,
        ...
        "attempts_unique": len({row["accepted_attempt"] for row in rows}) == 12,
    ...
    checks["derived_source_routing"] = (
        len(set(representative_sources.values())) == 1
    )
    ...
    flat = [
        checks["formal_cell_coverage_12_of_12"],
...
def t_summary(values: Sequence[float]) -> dict[str, Any]:
    if len(values) != 3:
        raise RuntimeError(f"three paired seeds are required, got {len(values)}")
    if not all(math.isfinite(value) for value in values):
        raise RuntimeError("paired values must all be finite")
    mean = statistics.mean(values)
    sample_sd = statistics.stdev(values)
    half_width = T_CRITICAL_95_DF2 * sample_sd / math.sqrt(3)
    return {
        "n_seeds": 3,
        "mean": mean,
        "sample_sd": sample_sd,
...
def method_summaries(rows: Sequence[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for method in EXPECTED_METHODS:
        ...
        if len(selected) != 3:
            raise RuntimeError(f"{method}: expected three rows")
        ...
        for metric in QUALITY_METRICS:
            ...
            classification = (
                practical_classification(
                    float(stats["ci95_low_t_df2"]),
                    float(stats["ci95_high_t_df2"]),
                    float(stats["mean"]),
                    margin,
                )
...
def build_decision(
    ...
    ambiguity = [
        row["contrast"]
        for row in primary
        if row["classification"] not in DEFINITIVE_PRACTICAL_CLASSIFICATIONS
    ]
    classifications = {
    ...
    return {
        ...
        "common_target_crossing_definition": (
            "first downward crossing of validation loss 2.95, linearly "
        ...
        "seed_append_rule": (
            ...
            "on the next unused seeds (2027 then 2028), at most two."
        ),
...
def build_report(
    ...
    lines = [
        ...
        "",
        "All 12 formal cells passed local integrity checks. The seed is the "
    ...
    for row in primary:
        lines.append(
            ...
            f"(95% t CI {row['ci95_low_t_df2']:+.6f}, "
            f"{row['ci95_high_t_df2']:+.6f}); {row['classification']}."
        )
    lines.extend(
        [
            "",
            ...
            "- CUDA peaks cover the counted run after warmup reset, including "
            "validation; they are not training-step-only peaks.",
            "- Newton-family K statistics aggregate all eight sequential "
            "single-H100 microbatches; strict equivalence to a hypothetical "
            "eight-rank owner-local K path is not claimed.",
...
def analyze(
    ...
    committed_manifest = output_dir / "record17_analysis_manifest.json"
    ...
    C.write_csv(output_dir / "record17_cells.csv", cells, cell_fields)
    C.write_csv(
    ...
    C.atomic_write_json(output_dir / "record17_decision.json", decision)
    C.atomic_write_text(
        output_dir / "RECORD17_ANALYSIS_REPORT.md",
    ...
    artifact_names = (
        ...
        "source_audit.json",
        "record17_decision.json",
        "RECORD17_ANALYSIS_REPORT.md",
    )
    manifest = {
        ...
        "contract_path": str(contract_path),
        "contract_sha256": contract_sha256,
        "accepted_formal_cells": 12,
        "expected_formal_cells": 12,
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    print(f"Record #17 analysis manifest: {output_dir / 'record17_analysis_manifest.json'}")
    print(f"Record #17 accepted formal cells: {manifest['accepted_formal_cells']}/12")
